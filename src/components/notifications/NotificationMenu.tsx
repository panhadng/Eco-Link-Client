  ...
  return (
    <div className="relative w-full md:w-auto" ref={menuRef}>
      <button
        type="button"
        onClick={toggleMenu}
        className="group relative flex w-full flex-col items-center justify-center py-1 md:flex-row md:min-w-0 md:rounded-xl md:p-2 md:inline-flex hover:bg-panel active:bg-panel md:hover:bg-panel focus:outline-none focus:ring-2 focus:ring-ring transition-colors"
        aria-label="Notifications"
      >
        <div className="relative flex items-center justify-center">
          <BellIcon className={`h-6 w-6 text-muted-foreground transition-opacity ${isOpen ? 'opacity-0' : 'opacity-100 md:group-hover:opacity-0'}`} />
          <BellIconSolid className={`absolute h-6 w-6 text-primary transition-opacity ${isOpen ? 'opacity-100' : 'opacity-0 md:group-hover:opacity-100'}`} />
          {unreadCount > 0 && (
            <span className="absolute -top-0.5 -right-1 md:-top-0.5 md:-right-0.5 flex h-5 min-w-[20px] items-center justify-center rounded-full bg-primary text-[10px] font-medium text-primary-foreground">
              {unreadCount > 99 ? '99+' : unreadCount}
            </span>
          )}
        </div>
        <span className="mt-0.5 text-xs leading-tight text-muted-foreground md:hidden">Notifications</span>
      </button>

      {isOpen && (
        <div className="fixed md:absolute right-4 md:right-0 left-4 md:left-auto bottom-20 md:bottom-auto mb-0 md:mt-3 md:top-full w-auto md:w-[500px] max-w-[calc(100vw-2rem)] md:max-w-[500px] rounded-xl border border-border bg-card shadow-lg z-50">
